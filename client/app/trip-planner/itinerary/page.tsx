...
const ItineraryPage = () => {
  ...
  const [selectedActivityType, setSelectedActivityType] = useState<
    'hotel' | 'activity' | 'attraction' | null
  >(null);
  ...
  const handleDateSelect = (date: string) => {
    ...
    const dayPlan = tripData.packages.packages[0]?.itinerary.find(day => day.date === date);
  ...
  const handleDeleteActivity = (dayDate: string, activityIndex: number) => {
    ...
    const updatedPackages = { ...tripData.packages };
    const dayIndex = updatedPackages.packages[0].itinerary.findIndex(day => day.date === dayDate);
  ...
  const renderActivityCard = (activity: any, dayDate: string, index: number) => {
    const getActivityIcon = (type: string) => {
      switch (type) {
        case 'hotel':
          return <FaHotel className="text-2xl text-teal-600" />;
        case 'food':
          return <FaUtensils className="text-2xl text-orange-600" />;
        case 'attraction':
          return <FaCamera className="text-2xl text-blue-600" />;
        default:
          return <FaWalking className="text-2xl text-green-600" />;
      }
    };
  ...
  const getSuggestions = (type: string, query: string = '') => {
    ...
    switch (type) {
      case 'hotel':
        return tripData.trip_details.hotels.filter(h => h.name.toLowerCase().includes(searchTerm));
      case 'activity':
  ...
  return (
    <div className="min-h-screen pb-32 bg-gradient-to-b from-teal-50 to-white">
      <div
        className="relative h-64 bg-cover bg-center"
        style={{
          backgroundImage: `url(${
            tripData.trip_details.attractions[0]?.image ||
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40">
          <div className="max-w-6xl mx-auto px-4 h-full flex flex-col justify-end pb-8">
            <h1 className="text-4xl font-bold text-white mb-2">
              Munnar {tripData.packages.packages[0]?.title || 'Your Trip Itinerary'}
            </h1>
      ...
      <div className="border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex gap-8 -mb-px">
            <div className="py-4 px-1 border-b-2 border-teal-500 text-teal-600">Itinerary</div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-4 mt-8">
          ...
          >
            {isSubmitting ? 'Submitting...' : 'Finish Trip & Submit'}
          </button>
        </div>
      </div>
