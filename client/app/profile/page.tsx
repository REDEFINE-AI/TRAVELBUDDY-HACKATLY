...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (authUser) {
      console.log('Auth User Subscription:', authUser.subscriptions);

      setUser({
        name: capitalizeWords(authUser.username || 'Guest User'),
        email: authUser.email || 'guest@example.com',
        subscription: authUser.subscriptions?.plan || 'Free Plan',
        credits: {
          aiPlanning: authUser.subscriptions?.trip_limit || 0,
          liveTranslator: authUser.subscriptions?.translator_limit || 0,
      ...
      <div className="bg-teal-50 rounded-xl p-4 mb-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center gap-2">
            <span className="bg-teal-600 text-white px-3 py-1 rounded-full text-sm capitalize">
              {user.subscription}
            </span>
            <span className="text-sm text-teal-600">
              Valid till {new Date(authUser?.subscriptions?.end_date || '').toLocaleDateString()}
            </span>
          </div>
        </div>
        ...
        <div className="flex gap-3 mt-4">
          <div className="flex-1 items-center gap-2 bg-white rounded-lg p-3">
            <div className="flex items-center gap-2 mb-1">
              <Image src="/coin.png" alt="Coins" width={20} height={20} />
              <span className="text-gray-600 text-sm">Coins</span>
            </div>
            <p className="text-xl font-semibold text-teal-800">{user.coins}</p>
          </div>
          <div className="flex-1 items-center gap-2 bg-white rounded-lg p-3">
            <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-xl font-semibold text-teal-800">${user.balance}</p>
          </div>
        </div>
      </div>
                    ...
                    <div className="flex items-start gap-4">
                      <div className="w-20 h-20 rounded-lg overflow-hidden relative bg-gray-100">
                        <Image
                          src={
                            trip.image ||
                            `/destinations/${trip.destination.toLowerCase().replace(', ', '-')}.jpg`
                          }
